...
## Use sort() to see the whole patterns
o<-list()
d<-list()
...
sdort.k12<- kmeans(sdort.new, centers = 12, nstart = 100)
sdort.k11$size
sdort.k12$size
kmeans(sdort.new, centers = 4, nstart = 100)$size
...
plot(sdortmc)
sdort.m12<-Mclust(sdort.new, G=12)

##### 3.5 MDS plots
mdsdort<- cmdscale(eusdort,k=2)
quartz(height = 12, width = 8)
par(mfrow=c(3,2))
plot(mdsdort,col=sdort.p12$cluster,pch=clusym[sdort.p12$cluster], main="pam_euclidean")
plot(mdsdort,col=sdort.k12$cluster,pch=clusym[sdort.k12$cluster], main="kmeans")
plot(mdsdort, col=sdortsing12,pch=clusym[sdortsing12], main="hclust_sing")
plot(mdsdort, col=sdortcomp12,pch=clusym[sdortcomp12], main="hclust_complete")
plot(mdsdort, col=sdortavg12,pch=clusym[sdortavg12], main="hclust_average")
plot(mdsdort, col=sdort.m12$classification, pch=clusym[sdort.m12$classification], main="mixture models")
par(mfrow=c(1,1))
dev.copy(pdf, "pam_kmeans_hclust_12_compare.pdf")
dev.off()
